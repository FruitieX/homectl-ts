...
export default class CircadianPlugin extends HomectlPlugin<Config> {
  ...
  getCurrentColorTemp = (): number => {
    const t = Date.now()
    const dayFadeStart = new Date().setHours(this.config.dayFadeStartHour, 0, 0, 0)
    const dayFadeEnd = new Date().setHours(this.config.dayFadeStartHour + this.config.dayFadeDuration, 0, 0, 0)

    const nightFadeStart = new Date().setHours(this.config.nightFadeStartHour, 0, 0, 0)
    const nightFadeEnd = new Date().setHours(this.config.nightFadeStartHour + this.config.nightFadeDuration, 0, 0, 0)

    if (t <= dayFadeStart || t >= nightFadeEnd) return this.config.nightColorTemperature
    if (t >= dayFadeEnd && t <= nightFadeStart) return this.config.dayColorTemperature

    if (t < dayFadeEnd) {
      // fading from night to day
      const p = (t - dayFadeStart) / (dayFadeEnd - dayFadeStart)
      return this.config.nightColorTemperature * (1 - p) + this.config.dayColorTemperature * p
    } else {
      // fading from day to night
      const p = (t - nightFadeStart) / (nightFadeEnd - nightFadeStart)
      return this.config.dayColorTemperature * (1 - p) + this.config.nightColorTemperature * p
    }
  ...
  async handleMsg(path: string, payload: unknown) {
    ...
    switch (cmd) {
      ...
      default: this.log(`Unknown cmd received: ${cmd}`)
    }
  }
}
