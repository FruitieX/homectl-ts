...
export abstract class HomectlPlugin<A> {
  ...
  constructor(
    { id, config, app, appConfig, sendMsg }: PluginProps<A>,
    decoder: t.Decoder<unknown, A>,
  ) {
    this.id = id;
    this.config = throwDecoder(decoder)(
      config,
      `Error while decoding integration config, quitting...`,
    );
    this.app = app;
...
class HomectlPluginImplementation<A> extends HomectlPlugin<A> {
  constructor(props: PluginProps<A>) {
    super(props, <any>IntegrationConfig);
  }
  async register() {}
}

export const loadPlugin = (props: PluginProps<IntegrationConfig>) => {
  const Plugin: typeof HomectlPluginImplementation = require(`${__dirname}/plugins/${props.config.plugin}`)
    .default;
