...
import { PluginProps, RoutineConfig, RoutinesConfig, throwDecoder, SensorUpdate } from "../types";
...
export default class RoutinesPlugin extends HomectlPlugin<Config> {
  ...
  async handleMsg(path: string, payload: unknown) {
    ...
    switch (cmd) {
      case "valueChange": {
        ...
        const sensorUpdate = throwDecoder(SensorUpdate)(payload, "Unable to decode valueChange payload")
        this.handleValueChange(sensorUpdate)
        break;
      }
      default: this.log(`Unknown command received: ${cmd}`)
    }
  }
  ...
  isRoutineMatch(sensorUpdate: SensorUpdate, routine: RoutineConfig) {
    return Boolean(routine.when.find(condition => {
      if (condition.path === sensorUpdate.path && condition.is === sensorUpdate.value) return true
      return false
    }))
  }
