...
export default class ScenesPlugin extends HomectlPlugin<Config> {
  ...
  async handleMsg(
    path: string,
    payload: unknown,
  ): Promise<Array<InternalDeviceCommand> | void> {
    const cmd = path;

    switch (cmd) {
      case 'getScene': {
        const sceneName = throwDecoder(t.string)(
          payload,
          'Unable to decode scene name',
        );
  ...
  async getScene(
    sceneName: string,
  ): Promise<Array<InternalDeviceCommand> | undefined> {
    const scene = this.scenes[sceneName];
    if (!scene) return;

    const sceneCommands: Array<InternalDeviceCommand> = [];
